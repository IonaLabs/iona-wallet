...
(def regx-string-universal-link
  (str "((^https?://ionalabs.ai/)|(^iona-labs://))"
       "("
       (string/join
        "|"
...
(def regx-deep-link #"((^ethereum:.*)|(^iona-labs://[\x00-\x7F]+)|(^wc://.*))")
...
(def ^:const token-sort-priority {"ETH" 1 "DAI" 2 "BNB" 3})
(def ^:const hidden-tokens #{"SNT" "STT"})
...
(def ^:const status-help-link "https://help.ionalabs.ai")
...
;; About app links
(def ^:const status-app-url "http://ionalabs.ai")
(def ^:const go-waku-url "https://x.com/ionalabs")
(def ^:const status-go-url "https://github.com/ionalabs")
(def ^:const status-mobile-url "https://docs.ionalabs.ai")
...
(def ^:const report-email "report@ionalabs.ai")

;; Community help links
(def ^:const create-community-help-url
  "https://help.ionalabs.ai/communities/create-a-iona-community")
